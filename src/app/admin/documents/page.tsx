...
import { DocumentCategoryService } from "@/service/docs-category.service"
import { EducationYearService } from "@/service/edu-years.service"
import type { Document, DocumentCategory, EducationYear } from "@/types"
...
export default function DocumentsPage() {
  ...
  const filteredDocuments = documents.filter((item) => item.title.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  return (
    <Container maxWidth="xl">
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} md={3}>
            <FormControl fullWidth>
              ...
              <Select value={selectedYear} onChange={handleYearChange} label="Учебный год">
            ...
            {isLoading ? (
              ...
              filteredDocuments.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>{categories.find((c) => c.id === item.categoryId)?.name || "Неизвестно"}</TableCell>
                  <TableCell>{years.find((y) => y.id === item.yearId)?.name || "Неизвестно"}</TableCell>
                  <TableCell>{new Date(item.createdAt).toLocaleDateString()}</TableCell>
                  <TableCell align="right">
                    <IconButton color="info" onClick={() => handleDownloadDocument(item.file)}>
                      <DownloadIcon />
                    </IconButton>
                    <IconButton color="primary" onClick={() => handleEditDocument(item.id)}>
                      <EditIcon />
                    </IconButton>
      ...
      <Dialog open={deleteDialogOpen} onClose={handleDeleteCancel}>
        ...
        <DialogContent>
          <DialogContentText>
            Вы уверены, что хотите удалить документ "{documentToDelete?.title}"? Это действие нельзя отменить.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
